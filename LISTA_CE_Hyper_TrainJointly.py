...
# parameter for HyperNet & Attention
Hyper_dim = [2, 128, 128, 2*max_iteration]  # HyperNet dimension
Hyper_dim_str = [str(x) for x in Hyper_dim]
...
appendix_tag_hyperModel = "_Hyper_" + '_'.join(Hyper_dim_str)
# appendix_tag_hyperModel = "_Hyper"
type_tag_aver = "CS" + CS_ratio_tag + "_layer" + str(max_iteration)
type_tag_hyper = "CS" + CS_ratio_tag + "_layer" + str(max_iteration)
model_dir_aver = 'LISTA_CE_' + type_tag_aver + appendix_tag_averModel
model_dir_hyper = 'LISTA_CE_' + type_tag_hyper + appendix_tag_hyperModel
output_file_name = "Log_output_%s.txt" % model_dir_hyper
...
def write_parameters_log(output_file_name, train_flag):
    ...
    if Train_flag == 1:
        ...
        Log_out("\n ### parameter for HyperNet ###")
        Log_out("HyperNet_dim = " + str(Hyper_dim))
        Log_out("\n ### parameter for Training ###")
        Log_out("Learning rate = " + str(lr))
        Log_out("model_dir_hyper = " + str(model_dir_hyper))
...
def ista_block(Hv, layer_num, W, s, Phi_tf, rho, theta):  # Construct half layer of LISTA_CE
...
def inference_ista():
    ...
    s_output = HyperInput_tf

    for ii in range(1, len(Hyper_dim)):
        with tf.variable_scope('HyperNet_layer%d' % ii, reuse=reuse):
            w = tf.get_variable('w', shape=[Hyper_dim[ii-1], Hyper_dim[ii]], initializer=tf.truncated_normal_initializer(stddev=0.05))
            b = tf.get_variable('b', shape=[Hyper_dim[ii]], initializer=tf.truncated_normal_initializer(stddev=0.05))
            if ii == len(Hyper_dim) - 1:
                Hyperoutput_tf = tf.matmul(s_output, w) + b
            else:
                s_output = tf.nn.relu(tf.matmul(s_output, w) + b)

    rho_layers = Hyperoutput_tf[0, :max_iteration]
    theta_layers = Hyperoutput_tf[0, max_iteration:]
    ...
    for i in range(max_iteration):
        Hv_hat_k = ista_block(Hv_hat[-1], i, W, s, Phi_tf, rho_layers[i], theta_layers[i])
        Hv_hat.append(Hv_hat_k)
    return Hv_hat, Hyperoutput_tf


def compute_cost(Hv_hat, Hv_init, Hyper_out_in):
    cost = []
    cost_rec = 0
...
def run_vali(sess, run_type="Vali", snr_in=10, l_in=3, No_random_flag=0):
    if No_random_flag != 1:# No_random_flag = 0时，随机选择
    ...
    Hyper_input = np.hstack((l_in, snr_in))
    ...
    Hv_out, Hyper_out_np = sess.run([[Hv_hat], Hyper_out], feed_dict={Hv_init: batch_xs, Phi_tf: Phi_input, HyperInput_tf: [Hyper_input], SNR: [snr_in]})
    Hv_out = np.asarray(Hv_out)
    Hyper_out_np = np.asarray(Hyper_out_np)
    Log_out("Hyper_out on l = " + str(l_in) + " SNR = " + str(snr_in) + str(Hyper_out_np))
...
def run_vali_AllPoint(sess, run_type="Vali"):
    ...
    if Train_flag == 1:
        ...
        np.savetxt("Array_" + model_dir_hyper + ".csv", NMSE_array, delimiter=",")
...
if __name__ == "__main__":
    ...
    with tf.Graph().as_default():
        ...
        HyperInput_tf = tf.placeholder(dtype=fdtype, shape=[1, Hyper_dim[0]])
        SNR = tf.placeholder(dtype=fdtype, shape=[1, ])

        Hv_hat, Hyper_out = inference_ista()
        cost_all = compute_cost(Hv_hat, Hv_init, Hyper_out)

        variable_Hyper = [v for v in tf.trainable_variables() if 'Hyper' in v.name]
        ...
        model_Hyper = tf.train.Saver(dict_Hyper)
        ...
        with tf.Session(config=config) as sess:
            ...
            LISTA_CE_save_dir = model_dir_aver + '/ReSaved_Model_LISTA_CE_epoch' + str(aver_epoch)
            All_save_dir = model_dir_hyper + '/Saved_Model'
            if Train_flag == 0:
                save_dir = All_save_dir + '/Saved_Model_LISTA_CE_epoch' + str(max_episode)
                model_All.restore(sess, save_dir)
                run_vali(sess)
                run_vali_AllPoint(sess)
            else:
                print("LISTA_CE_save_dir = ", LISTA_CE_save_dir)
                ...
                for epoch_i in range(max_episode + 1):
                    ...
                    loss_batch = list()
                    rand_inds = np.random.choice(Training_inputs.shape[1], Training_inputs.shape[1], replace=False)
                    for i in range(Training_inputs.shape[1] // train_batchsize):
                        Phi_input = phi_gen()
                        snr = SNR_sample[random.randint(0, len(SNR_sample) - 1)]
                        l = L_set[random.randint(0, len(L_set) - 1)]
                        Hyper_input = np.hstack((l, snr))
                        batch_xs = Training_inputs[l - L_set[0]][rand_inds[i * train_batchsize:(i + 1) * train_batchsize]][:]
                        _, cost_ = sess.run([opt, cost_all[max_iteration - 1]], feed_dict={Hv_init: batch_xs, Phi_tf: Phi_input, HyperInput_tf: [Hyper_input], SNR: [snr]})
                    ...
                    if epoch_i <= 1:
                        save_dir = All_save_dir + '/Saved_Model_LISTA_CE_epoch' + str(epoch_i)
